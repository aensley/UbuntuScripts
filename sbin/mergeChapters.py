...
if len(sys.argv) < 4:
    print("Usage:")
    print("{} [input file] [input file] [output file]".format(os.path.basename(sys.argv[0])))
    print("")
...
START={int(c['start'] * 1000000000)}
END={int(c['end'] * 1000000000)}
title={c['title']}"""
...
result = subprocess.run(
    ["ffmpeg", "-f", "concat", "-safe", "0", "-i", fileListFileName, "-i", metadataFileName, "-map_metadata", "1", "-map", "0", "-c", "copy", "-c:a", "copy", "-c:s", "copy", sys.argv[3]],
